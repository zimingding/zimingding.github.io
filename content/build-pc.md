...
There is website that allows you to list all the parts and automatically try to fetch the best price from various vendors.
https://au.pcpartpicker.com/list/zjkD3R
...
Build experience is kind of fun, you can think of all the parts as lego pieces and pc the finished product. ChatGPT as usual and YouTube provided great guidance. You don't need to be very handy to do this job just need to be a bit careful to not accidentally damage pc parts.
...
Fortunately I found a YouTube video showing exact CPU Cooler model of mine and how to assemble so it went quite well.

### RAM

Apart from the price, RAM is very easy to install.

### SSD

Another easy to install part, there is an SSD fixer to make it stay where it is.

### Case

Before putting in the GPU I decided to hang the motherboard with its children to the case first. I never knew how it worked until I did it once. Its just simple screw on, only to be careful to have the I/O plate sitting in the motherboard i/o ports when fixing it in the case.

### GPU

Another easy to install part
...
When I later changed PSU from ASUS to Corsair the advantage is quite obvious - Corsair is using modular design which means I only connect the connector I need to the power while ASUS just bundle with all the connectors.

From the case there are a few wires need to go to motherboard:
- power on connector (so that we can press the button to turn on the pc)
- USB connector
- Audio connector

### Quick Test

Fortunately for me everything worked so well in my first attempt, I managed to turn on the PC and entered BIOS mode, in BIOS I am happily see the CPU/RAM/GPU
